...
namespace HelloWall
{
    class Program
    {
        ...
        static int Main()
        {
            ...
            //1 уровнь в дереве построения это model = IfcStore
            //далее можно либо создать новый файл, либо изменить
            //или удалить существующий файл
            using (var model = CreateandInitModel("HelloWall"))
            {
                if (model != null)
                {
                    //2 уровень в дереве это Ifc Building
                    IfcBuilding building = CreateBuilding(model, "Default Building");
                    IfcBuilding building2 = CreateBuilding(model, "Default Building1");

                    //3 уровень в дереве построения это части IfcBuilding
                    //типа стен, окон, любой произвольной геометрии
                    IfcWallStandardCase wall = CreateWall(model, 0,0,0, "wall1");
                    IfcWallStandardCase wall2 = CreateWall(model, 50, 50, 50, "wall2");
                    IfcWallStandardCase wall3 = CreateWall(model, -50, -50, -50, "wall3");

                    
                    using (var txn = model.BeginTransaction("Add Wall"))
                    {
                        //вот так заполняются уровни
                        building.AddElement(wall);
                        building2.AddElement(wall2);
                        building2.AddElement(wall3);
                        txn.Commit();
                    }

                    //теперь объектам 3го уровня можно задать свойства
                    //и количество этих объектов
               
                    using (var txn = model.BeginTransaction("Add Property"))
                    {
                        //создание свойства
                        CreateSimpleProperty(model, wall);
                        CreateSimpleProperty(model, wall2);
                        CreateSimpleProperty(model, wall3);

                        //создание количество
                        CreateElementQuantity(model, wall);
                        CreateElementQuantity(model, wall2);
                        CreateElementQuantity(model, wall3);

                        txn.Commit();
                    }

                    //констукция выше есть отдельным методом
                    //if (wall != null) AddPropertiesToWall(model, wall);
                    //if (wall2 != null) AddPropertiesToWall(model, wall2);
                    //if (wall3 != null) AddPropertiesToWall(model, wall3);
                    ...
                    if (wall != null)
                    {
                        try
                        {
                            ...
                            //write the Ifc File
                            model.SaveAs(@"C:/Test.ifc", StorageType.Ifc);
                            Console.WriteLine("C:/Test.ifc has been successfully written");
                        }
                        catch (Exception e)
                        {
        ...
        /// <returns></returns>
        static private IfcWallStandardCase CreateWall(IfcStore model, double PosX, double PosY, double PosZ, string name)
        {
            //Тут я немного поменял метод, чтобы двигать стену
            //begin a transaction
            using (var txn = model.BeginTransaction("Create Wall"))
            {
                ...
                insertPoint.SetXYZ(PosX, PosY, PosZ); //insert at arbitrary position
